...
def load_data(data_path: str, city: str, shop: str, day: str = "1-Mon") -> \
    ...
    # muze existovat vice suboru v cilovem adresari
    sDir = data_path + os.path.sep + city + os.path.sep + day

    if not os.path.exists(sDir):
        return None

    sFile = sDir + os.path.sep + shop + ".txt"
    if os.path.exists(sFile):
        with open(sFile, "r") as f:
            ...
            for i in range(1, len(lines)):
                    line = lines[i].strip()
                    if line:
                        time, dum1, id_cust, dum2 = line.split(';')
                        if time not in city_data:
                            city_data[time] = []
                        city_data[time].append(Record(int(time), int(id_cust)))
...
def histogram(data: dict[str, list[Record]]) -> None:
    for time in data:
        print(time, get_q_size(data, time))
